...
-- ]]Locking mechanishm
-- Automatically applies locks but user can change them
-- types:
-- row level, table level, advisory - custom locks

-- ]]mvcc vs locks

-- readers and writers co-exist without blocking
-- writers may block other writers while updting a table or a row

-- ]]simple rule for locks
-- ***readers need not block each other
-- ***writers block other writers for the same row

-- row level locking - default behaviour
...
begin;
update tbl_accounts set balance=5000 where id=3;

---transaction B(run in different session)
select * from tbl_accounts;
begin;
update tbl_accounts set balance=5500 where id=3;

-- transaction b waits until transaction a commits or rollbacks

-- table level

-- lock tablename in modename mode;

-- 1. Access share: allows read and write
-- 2. Row share: (select for update), reads are allowed,conflicting rows are blocked
-- 3. Exclusive: blocks writes, allows read
-- 4. Access exclusive: aggressive, locks everything


-- deadlock: two transaction wait for each other to commit or rollback: hence wait forever
example:
transaction A
begin;
...
transaction B
update tbl_accounts set balance=5500 where name='Alice';
-- locks alice

transaction A
update tbl_accounts set balance=5000 where name='Alice';
-- tries to locks Alice which is already locked by B

transaction B
update tbl_accounts set balance=5000 where name='Chad';
-- tries to locks chad which is already locked by A

-- creates a deadlock...
-- pgsql detects a deadlock: ERROR: Deadlock detected

-- it removes a transaction to resolve deadlock
